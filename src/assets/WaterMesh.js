...
class WaterNode extends TempNode {
	...
	setup() {
		...
		const outputNode = Fn( () => {
			...
			const ringNoiseIntensityMask = length(uv().sub(0.5).mul(2))
			ringNoiseIntensityMask.smoothstepAssign(0.2, 0.8)
			ringNoiseIntensityMask.remapAssign(0, 1, 0, 2)

			const ring = length(positionWorld.xz.add(normal.xz.mul(ringNoiseIntensityMask)))
			ring.subAssign(time.mul(0.35))
			ring.mulAssign(1.3)
			ring.sinAssign()
			ring.absAssign()
			ring.smoothstepAssign(0.5, 1)

			// return ring

			// return vec4(1, 0, 0, 1)
			const colorA = vec4( this.color, 1.0 ).mul( mix( refractionSampler, reflectionSampler, reflectance ) );
			const colorB = reflectionSampler.rgb.oneMinus().toVec4()

			colorA.a.mulAssign(ring)
			return colorA
			return mix(colorA, colorB, ring)
		} )();
